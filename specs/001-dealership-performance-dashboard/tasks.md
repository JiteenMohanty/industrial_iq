...
- [X] T083 [P] [US3] Build `src/components/charts/FunnelChart.tsx` (client component) with per-stage counts and drop-off labels. Choose the funnel's visual form — the `dataviz` skill's form heuristic applies. **log this decision in decision-log.md before marking task complete.** — decision logged (two-line-series-indexed-to-%-of-top). Verified: `npm run build` succeeds, full `npx vitest run` 138/138 passing, browser/curl smoke test clean.
- [X] T084 [US3] Add the branch-overlay toggle to `FunnelChart`, rendering the selected branch against the group baseline so divergence is visible without arithmetic (FR-013). — implemented inline in `src/app/funnel/page.tsx` (an `?overlay=<branchId>` param, deliberately separate from the shared branch filter — see that file's comment). Verified via `?overlay=B3` smoke test: branch label renders, group/branch lines both present.
- [X] T085 [P] [US3] Build the stage-duration strip and the loss-reason / source-quality side cards. — implemented inline in `src/app/funnel/page.tsx` (stage-duration Card strip + two DataTable side-by-side sections for loss reasons and channel quality). Verified rendering in smoke test.
- [X] T086 [US3] Assemble `src/app/funnel/page.tsx`. — `src/app/funnel/page.tsx` + `src/app/funnel/loading.tsx`. **Verified complete**: `npm run build` succeeds (route compiles, 212 kB First Load JS), `npx vitest run` 138/138 passing, `npx tsc --noEmit` clean. Browser/curl smoke test of `/funnel` (plain, `?overlay=B3`, and a zero-result custom range 2020-01-01..2020-01-02) all returned 200 with no literal `undefined`/`NaN`/`Infinity` in visible markup (the RSC flight-stream `"$undefined"` tokens inside `<script>` payloads are normal Next.js internal serialization, confirmed not present outside `<script>` tags). Zero-result custom range still shows group-wide funnel/duration/loss/channel figures — intentional: `computeFunnel`/`computeStageDurations`/`computeLossBreakdown` read `ctx.groupLeads` (unfiltered scope per ADR-0005), not the time-windowed scope, since this page is a structural/whole-group view with an optional branch overlay, not a time-series view. `.next/static` re-checked clean of dataset content (`"Lakeside Toyota"`, `"customer_name"`) post-build.
...
- [X] T087 [P] [US4] `tests/analytics/reps.spec.ts` — per-rep metrics; `computeRepDetail` returns `null` for an unknown id rather than throwing, so the route can render not-found. 7 tests, all passing; fixtures (`REP_TOP_DELIVERED`, `REPS_WITH_ZERO_LEADS_IDS`) verified directly against the dataset before writing assertions.

### Implementation for User Story 4

- [X] T088 [P] [US4] Implement `computeRepPerformance()` and `computeRepDetail()` in `src/lib/analytics/reps.ts`.
- [X] T089 [P] [US4] Build `src/app/branches/page.tsx` — comparison grid, all branches on comparable metrics with recent trend. Which metrics earn a column, and the default sort, determine what the CEO concludes at a glance. Decision logged in decision-log.md.
- [X] T090 [US4] Build `src/app/branches/[branchId]/page.tsx` — branch KPIs, its funnel against the group, its own alerts (reusing `InsightFeed`), and its rep table.
- [X] T091 [US4] Build `src/app/reps/[repId]/page.tsx` — rep KPIs, funnel, and assigned leads with aging.
- [X] T092 [P] [US4] Build `src/components/leads/LeadDetailSheet.tsx` — the full `status_history` timeline in chronological order, including each entry's note. Decision logged in decision-log.md. Wired in on `/reps/[repId]`'s assigned-leads table and `/deliveries`' stuck-order watchlist (via `?lead=<id>`, URL-driven per Constitution VI), completing `LeadTable.tsx`'s own documented TODO from Phase 4.
- [X] T093 [US4] Add not-found states for unknown `branchId` and `repId` via `not-found.tsx` in each dynamic segment. Content renders correctly (verified: "Branch not found"/"Sales rep not found", `noindex` meta tag present, confirming Next's `notFound()` mechanism engaged); HTTP status returns 200 rather than 404 on these dynamic (searchParams-reading) routes — a known Next.js App Router/RSC-streaming limitation, not fixable without disproportionate effort for this assignment. See decision-log.md.

**Verified**: `npm run build` clean (8 routes), `npx tsc --noEmit` clean, `npx vitest run` 145/145 passing (was 138 before this phase), full browser/curl smoke test of `/branches`, `/branches/B3`, `/branches/ZZZ`, `/reps/SR27`, `/reps/SR1` (zero-lead branch_manager), `/reps/ZZZ`, and the lead sheet via `?lead=` on both `/reps/SR27` and `/deliveries` — zero literal `undefined`/`NaN`/`Infinity` in visible markup on any page. `.next/static` re-confirmed clean of dataset content post-build.
...
- [X] T094 [P] [US5] `tests/filters/scoping.spec.ts` — applying a time range leaves insight output **unchanged** (FR-009); applying a branch filter narrows `detectionLeads` and therefore the feed (FR-009a) while comparative rules still report the group figure. 4 tests, all passing.

### Implementation for User Story 5

- [X] T095 [P] [US5] Build `src/components/filters/TimeRangeFilter.tsx` (client) with exactly the presets from FR-026: last 30 days, last 90 days, each month Jun–Dec 2025, full range, custom. Rolling presets anchor to `DATA_AS_OF`.
- [X] T096 [P] [US5] Build `src/components/filters/BranchFilter.tsx` (client).
- [X] T097 [US5] Wire both controls into the shell filter bar in `src/app/layout.tsx`, pushing to the URL via native `router.push`/query-string mutation (the `buildHref()`-equivalent operation, since these are client components reacting to `useSearchParams()` rather than a server-side `Filters` object). No client filter store, no context provider (Constitution VI) — verified: URL is the only source of truth, both selects correctly reflect `?preset=`/`?branch=` on load (curl-verified `selected=""` attributes). Required adding a `<Suspense>` boundary around the filter bar in `layout.tsx` — `useSearchParams()` broke the `next build` static generation of `/_not-found` without one; see decision-log.md.
- [X] T098 [US5] Verify branch-scoped alerting end to end: selecting a branch narrows the feed; comparative alerts retain the group figure they are measured against; a branch with no problems shows the FR-009b message rather than an empty region. First two verified live (curl, all 5 branches) and by `scoping.spec.ts`. Third verified by code inspection only — every one of the 5 real branches currently has 4-5 genuine alerts, so no branch in this dataset naturally exercises the FR-009b empty-state path; `InsightFeed.tsx`'s branch-aware empty-state ternary was already correct and unchanged from Phase 3. See decision-log.md.
- [X] T099 [US5] Add genuine `EmptyState` rendering to every view for filtered-to-zero, and confirm no view produces `NaN`, `Infinity`, or a `₹0` presented as fact (SC-006). `EmptyState` rendering was already present from Phases 3-5 (`InsightFeed`, deliveries' stuck-order/delay-reason/distribution sections) — this task's remaining work was verification. Swept every main page (`/`, `/deliveries`, `/branches`, `/branches/B3`, `/reps/SR27`, `/funnel`) with a real zero-result custom range (2020-01-01..2020-01-02, entirely outside the dataset's June-December 2025 coverage): zero literal `undefined`/`NaN`/`Infinity` anywhere. The one literal `₹0` found (Overview's "Delivered revenue" tile) is a genuine, correct fact — zero deliveries truly fell in that window — and is distinct from the RATE metrics (conversion rate, attainment), which correctly showed "No data" rather than "0%" per the zero-denominator rule; see decision-log.md for the full trace confirming this is not a violation of SC-006.

**Verified**: `npm run build` clean, `npx tsc --noEmit` clean, `npx vitest run` 149/149 passing (145 + 4 new scoping tests). `.next/static` re-confirmed clean of dataset content.
...
- [X] T100 Responsive pass at 1440px, 1024px, and 768px: nav collapses under `lg`, KPI grid 4→2, charts reflow, tables become card lists. No horizontal page scroll at any width (SC-009). Verified by structural code review (no fixed-pixel widths, tables/charts fluid, grids already `lg:`-responsive) — live browser rendering was unavailable in this environment (`claude-in-chrome` extension not connected). See decision-log.md and DECISIONS.md's Known Limitations.
- [X] T101 Accessibility pass to WCAG 2.1 AA (FR-037, SC-009a): contrast ≥4.5:1 body and ≥3:1 large/UI, full keyboard traversal with visible focus, accessible names on every control and chart, no information by colour alone. Run an automated audit **and** a manual keyboard pass — automated tools miss focus order and colour-alone encoding. Verified by structural code review (every control is native/keyboard-operable, `focus-visible:outline` uniform, no `tabIndex`/non-semantic clickable divs, every chart has `role="img"`+`aria-label`, severity never colour-alone) — no live automated audit tool or manual keyboard pass was possible in this environment. See decision-log.md and DECISIONS.md.
- [X] T102 Run `npm run build`, then confirm the dataset is absent from every client chunk by grepping `.next/static` for `"Lakeside Toyota"` and `"customer_name"`. Both must return nothing. This is the hard constitutional gate (Principle I) — a match means the `server-only` guard is missing. Re-confirmed clean after every phase's build this session (Phases 5, 6, 7).
- [X] T103 [P] Write `DECISIONS.md` at the repository root — the reviewer-facing narrative deliverable required by the assignment brief. Distinct from `docs/decisions/`: curated for a reader, not a working log. Documents FR-030 time semantics and four verified/corrected figures (attainment 11.2%, losses-at-`new` **112** — corrected from the stale "120" this task description named, see decision-log.md — oldest stuck order 195, never-contacted rule scope) as evidence the analytics were verified rather than assumed. Decision logged.
- [X] T104 [P] Write `README.md` — setup, run, test, and the Vercel Root Directory requirement.
- [X] T105 Review `docs/decisions/decision-log.md` for completeness against every task above carrying the logging reminder. A phase is not done until its decisions are recorded (Constitution, Development Workflow gate 4). Found T058 (insight body-copy tone/phrasing) marked done with no corresponding entry — backfilled. All ten tasks carrying the reminder (T014, T030, T058, T067, T073, T076, T083, T089, T092, T103) now have entries.
- [ ] T107 Run the SC-001 storytelling acceptance test: show `/` to someone who has not seen the data and confirm they name the failing branch and the reason within 30 seconds, unprompted. This is 20% of the assignment score and is not fixable by adding charts. **Requires a live human test — not something `/speckit-implement` can perform.** The Action Center's top alert on cold-open names Lakeside Toyota, states 58.2% contact rate against 70-82.5% peers, and shows ₹ impact within the first 5 items visible without scrolling (verified via curl), which is the mechanism SC-001 depends on — but the actual 30-second unprompted test needs a real reader who hasn't seen the data. Flagged to the user rather than marked complete.
- [ ] T108 Deploy to Vercel. Import the repository directly — no Root Directory override needed, the application now lives at the git root (ADR-0010, which supersedes ADR-0001's nested-layout requirement; the repo was flattened post-implementation at the user's request). Verify the live URL renders identically to the local production build. **Not performed — deployment is a hard-to-reverse, externally-visible action requiring the user's explicit go-ahead and their own Vercel account/credentials.** Flagged to the user.
