...
Application root was `C:\Projects\Assignment\Assignment` (the Spec Kit root) during implementation
— see ADR-0001 for why. **Post-implementation, the repository was flattened at the user's request**
(ADR-0010): the application now lives at `C:\Projects\Assignment`, the git root, and no Vercel
root-directory override is needed. The tree below is otherwise unchanged — only its base path moved
up one level.
