...
- The dataset at `../docs/dealership_data.json` (copied to `src/data/` during scaffold; the
  original is never modified)

## Setup

```bash
# from C:\Projects\Assignment\Assignment  (the application root — see ADR-0001)
npm install
...
Vercel project **Root Directory must be set to `Assignment`** — the git root is one level above the
application (ADR-0001, research R8). Without it the build will not find `package.json`.
